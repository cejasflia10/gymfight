...
if (isset($_GET['eliminar'])) {
    ...
    $conexion->query("DELETE FROM productos_protecciones WHERE id = $id");
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    $stmt = $conexion->prepare("INSERT INTO productos_protecciones (nombre, talle, precio_compra, precio_venta) VALUES (?, ?, ?, ?)");
    $stmt->bind_param("ssdd", $nombre, $talle, $precio_compra, $precio_venta);
    $stmt->execute();
    $mensaje = "Producto agregado.";
}

$productos = $conexion->query("SELECT * FROM productos_protecciones ORDER BY nombre");
?>

<div class="container mt-4">
  <h2 class="mb-4">Productos - Protecciones</h2>
  <?php if ($mensaje): ?>
  ...
  <form method="POST" class="mb-4 row g-2">
    ...
    <div class="col-md-2"><input type="text" name="talle" placeholder="Talle/Oz" class="form-control" required></div>
